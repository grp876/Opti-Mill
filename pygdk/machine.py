...
################################################################################
    ...
    def helix(self, c_x, c_y, diameter, depth, z_step=10, outside=False, retract=True):
        if self._optimize:
        ...
        self.queue(code='G2', z=-depth, i=self.tool.diameter/2-diameter/2, j=0, p=int(z_step), f=self.feed, comment='Heli-drill')
...
################################################################################
    ...
    def generate_gcode(self):
        ...
        for command in self.command_queue:
            ...
            if command.get('s',None) is not None:       # Spindle RPM
                line += f" S{command['s']:.4f}"
            if command.get('comment',None) is not None: # Human-readable comments
                #GRP line += f"; {styles[command.get('style', '')]}{command.get('comment', '')}{ENDC}"
